...
export default function FeaturesPage() {
  ...
  const copy = fa
    ? {
        ...
        intro:
          "زبدینو برای این ساخته شده که یک سند یا کتاب را فقط خلاصه نکند؛ بلکه نتیجه‌ای بدهد که بتوانی بخوانی، منبعش را بررسی کنی، گوش بدهی و بعداً دوباره به آن برگردی.",
        primary: "تبدیل فایل",
        secondary: "دیدن کتاب‌ها",
        journeyTitle: "تجربه‌ای که از ابتدا تا انتها به هم وصل است",
        trustTitle: "چرا این تجربه متفاوت است؟",
        trustText:
          "خلاصه، Evidence، صوت و کتابخانه خصوصی اجزای جدا از هم نیستند. هر خروجی به همان فایل و همان مسیر پردازش متصل می‌ماند تا تجربه قابل اعتماد و قابل ادامه باشد.",
      }
    : {
        eyebrow: "Zobdino features",
        title: "From a raw file to understanding, evidence and audio in one flow.",
        intro:
          "Zobdino is built to do more than summarize a document or book. It gives you an output you can read, verify against its source, listen to, and reopen later.",
        primary: "Upload a file",
        secondary: "Browse books",
        journeyTitle: "One connected experience from start to finish",
        trustTitle: "What makes the workflow different?",
        trustText:
          "Summary, evidence, audio and the private library are not separate tools. Each output stays connected to the same source file and processing journey so the experience remains trustworthy and resumable.",
      };
  ...
  const journey = fa
    ? ["فایل را وارد کن", "خلاصه و Evidence را ببین", "صدا را گوش بده", "در کتابخانه خصوصی نگه دار"]
    : ["Upload your file", "Review summary and evidence", "Listen to audio", "Keep it in your private library"];
  ...
  return (
    <main className="z-container py-10 md:py-16">
      <section className="grid gap-8 lg:grid-cols-[1fr_390px] lg:items-end">
        <div className="max-w-3xl">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <Link href="/books" className="z-focus inline-flex items-center gap-2 rounded-2xl border border-black/10 bg-white/65 px-5 py-3.5 text-sm font-black dark:border-white/10 dark:bg-white/[0.035]">
              {copy.secondary}
            </Link>
          </div>
        </div>
        ...
        <div className="z-surface p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-black">{copy.journeyTitle}</p>
              <ol className="mt-4 space-y-3">
                {journey.map((item, index) => (
                  <li key={item} className="flex items-center gap-3 text-sm z-muted">
                    <span className="grid size-7 shrink-0 place-items-center rounded-full bg-black/[0.045] text-xs font-black dark:bg-white/[0.06]">
      ...
      <section className="mt-14 md:mt-18">
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {features.map(([Icon, title, text]) => {
            ...
            return (
              <article key={String(title)} className="z-surface p-6">
                ...
                <p className="mt-3 text-sm leading-7 z-muted">{String(text)}</p>
              </article>
            );
          })}
        </div>
      ...
      <section className="z-surface mt-14 overflow-hidden p-7 md:p-9">
        <div className="grid gap-7 md:grid-cols-[1fr_auto] md:items-center">
          <div className="max-w-2xl">
            <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-300">
              <LockKeyhole size={18} />
              <p className="text-sm font-black">{copy.trustTitle}</p>
            </div>
            <p className="mt-4 text-base leading-8 z-muted">{copy.trustText}</p>
          </div>
          <Link href="/upload" className="z-focus inline-flex items-center gap-2 rounded-2xl bg-[var(--page-fg)] px-5 py-3.5 text-sm font-black text-[var(--page-bg)]">
            {copy.primary}
            <Arrow size={16} />
          </Link>
        </div>
